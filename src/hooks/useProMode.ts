...
export const useProMode = () => {
  const { options, testProxyProMode, testProxiesProModeBatch } = useProxyTesterStore();
  ...
  const testSingleProxyProMode = useCallback(async (proxy: string) => {
    if (!isProModeEnabled) {
  ...
  const testMultipleProxiesProMode = useCallback(async (proxies: string[]) => {
    if (!isProModeEnabled) {
      throw new Error('Pro Mode is not enabled');
    }
  ...
  const getProModeStats = useCallback(() => {
    ...
    const avgFirstConnection = workingProxies.length > 0 
      ? workingProxies.reduce((sum, r) => sum + r.firstConnectionTime, 0) / workingProxies.length
      : 0;

    const avgSubsequentConnection = workingProxies.length > 0 
      ? workingProxies.reduce((sum, r) => sum + (r.subsequentConnectionTime || r.firstConnectionTime), 0) / workingProxies.length
      : 0;

    const avgDns = workingProxies.length > 0
      ? workingProxies.reduce((sum, r) => sum + r.averageMetrics.dnsLookupTime, 0) / workingProxies.length
      : 0;

    const avgTcp = workingProxies.length > 0
      ? workingProxies.reduce((sum, r) => sum + r.averageMetrics.tcpConnectTime, 0) / workingProxies.length
      : 0;

    const avgTls = workingProxies.length > 0
      ? workingProxies.reduce((sum, r) => sum + r.averageMetrics.tlsHandshakeTime, 0) / workingProxies.length
      : 0;

    const avgProxy = workingProxies.length > 0
      ? workingProxies.reduce((sum, r) => sum + r.averageMetrics.proxyConnectTime, 0) / workingProxies.length
      : 0;

    const avgResponse = workingProxies.length > 0
      ? workingProxies.reduce((sum, r) => sum + r.averageMetrics.responseDownloadTime, 0) / workingProxies.length
      : 0;
    ...
    return {
      ...
      successRate: proModeResults.length > 0 ? (workingProxies.length / proModeResults.length) * 100 : 0,
      averageMetrics: {
  ...
  const getDetailedProxyMetrics = useCallback((proxy: string) => {
    const result = proModeResults.find(r => r.proxy === proxy);
    return result?.connections || [];
  }, [proModeResults]);

  const exportProModeResults = useCallback((format: 'json' | 'csv' = 'json') => {
    if (proModeResults.length === 0) {
      return null;
    }
    ...
    // CSV format
    const headers = [
      'Proxy', 'Status', 'Exit IP', 'Country', 'City', 'ISP',
      'Test Method', 'Connections Count', 'First Connection (ms)', 'Subsequent Connection (ms)',
      'DNS (ms)', 'TCP (ms)', 'TLS (ms)', 'Proxy Connect (ms)', 'Response (ms)', 'Total Avg (ms)'
    ];
